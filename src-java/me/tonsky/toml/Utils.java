...
public final class Utils {
	...
	public static Object getIn(IPersistentMap m, List path, Object notFound) {
		Object key = path.get(0);
		if (path.size() == 1) {
			return m.valAt(key, notFound);
		} else {
			Object m2 = m.valAt(key, notFound);
			if (m2 == notFound) {
				return notFound;
			}
			return getIn((IPersistentMap) m2, path.subList(1, path.size()), notFound);
		}
	}

	public static IPersistentMap assocIn(IPersistentMap m, List path, Object value) {
		Object key = path.get(0);
		if (m == null) {
		...
		if (path.size() == 1) {
			...
			return m.assoc(key, assocIn((IPersistentMap) m.valAt(key), path.subList(1, path.size()), value));
		}
	}
